...
addedJs = {};
Object.keys(window).filter(arrObj => defaultWindowArray.every(obj => obj != arrObj)).filter(arrObj => arrObj != "defaultWindowArray").forEach(arrObj => addedJs[arrObj.toString()] = window[arrObj]);
console.log(addedJs);
```
