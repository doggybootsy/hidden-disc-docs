...
addedJs = [];
console.log("gathered everything into an array!");
Object.keys(window).filter(arrObj => defaultWindowArray.every(obj => obj != arrObj)).filter(arrObj => arrObj != "defaultWindowArray").forEach(arrObj => addedJs.push(window[arrObj]));
console.log(addedJs);
```
